...
This is running over leaflet and is a convenient way to view your spatial data.
...
st_crs(fire_perims)
```
EPSG assigns a number to every map projection - 4269 is the NAD83 projection which you can look up. This is a universal numbering system. Datum (NAD83)
If proj=longlat, the coordinates are not flat (they are in lat/long) so you need to somehow make it flat.
...
fire_intersections <- st_intersection(ca_nps_buffer, fire_proj)
mapview(fire_intersections)

mapview(ca_nps_buffer)+
  mapview(fire_proj, color = "orange")
```
...
One thing I did not cover was how to make maps (except in our interactive `mapview` outputs). For that I would again defer to Jamie Montgomery's [guide](https://github.com/jamiecmontgomery/spatial-analysis-R) on `sf`. Look here for info on making colorful maps and coding color into your map

```{r mapping}
library(ggplot2)

ca <- read_sf("data/state/california2016.shp") #read in an outline for the state of california

ggplot()+
  geom_sf(data = ca)+
  geom_sf(data = ca_nps, color = "darkgreen")+
  geom_sf(data = fire_perims, color = "orange", fill = "orange", alpha = 0.5) #alpha sets transparency

#should project california
...
ggplot()+
  ...
  geom_sf(data = fire_proj, color = "orange", fill = "orange", alpha = 0.5) #alpha sets transparency
```
